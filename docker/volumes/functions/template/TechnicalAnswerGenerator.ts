...
export class TechnicalAnswerGenerator extends AbstractGenerator {
    protected async getContext(message: Message, products: Product[]): Promise<string> {
        ...
        for (const doc of documents) {
            if (doc.getName().includes('FI')) {
                filteredDocuments.push(doc);
            }
        }
        if (filteredDocuments.length === 0) {
            filteredDocuments.push(documents[0]);
        }

        const chunk: Chunk[] = await this.services.obtainSimilarChunkUseCase.obtainSimilarChunk(message, filteredDocuments, 5);
        let context: string = "";
        if(!chunk) {
