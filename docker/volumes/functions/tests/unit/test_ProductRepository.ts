...
Deno.test("ProductRepository: getAllProduct should throw an error if database query fails", async () => {
  // Arrange
  const mockClient = {
    from: () => ({
      select: async () => ({
...
Deno.test("ProductRepository: getProduct should return a valid SupabaseProduct by ID", async () => {
  // Arrange
  const mockClient = {
    from: () => ({
      select: () => ({
  ...
  // Act
  const result = await repository.getProduct(null, "1");
...
Deno.test("ProductRepository: getProduct should throw an error if query result is null", async () => {
  // Arrange
  const mockClient = {
    from: () => ({
      select: () => ({
        eq: async () => null, // Simula risultato nullo
      }),
    }),
  };
...
Deno.test("ProductRepository: getProduct should throw an error if result.data is empty", async () => {
  // Arrange
  const mockClient = {
    from: () => ({
      select: () => ({
...
Deno.test("ProductRepository: getProduct should throw an error if product is null", async () => {
  // Arrange
  const mockClient = {
    from: () => ({
      select: () => ({
  ...
  try {
    ...
    if (error instanceof Error) {
      assertEquals(error.message, "Product not found"); // Ensure this matches the actual error message
    }
  }
});