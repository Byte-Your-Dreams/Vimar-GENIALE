...
Deno.test("Integration - GenerateAnswerUseCase - generateAnswer - success", async () => {
    ...
    assertEquals(result.getAnswer(), "Generated answer", "Should return the expected answer");
});

Deno.test("Integration - GenerateAnswerUseCase - generateAnswer - failure", async () => {
    const mockPort = createMockGenerateAnswerPort(true); // Simula un errore
    const useCase = new GenerateAnswerUseCase(mockPort);
    const prompt = new AIPrompt([{ role: "user", content: "What is AI?" }]);

    await assertRejects(
        () => useCase.generateAnswer(prompt),
        Error,
        "Generation failed",
