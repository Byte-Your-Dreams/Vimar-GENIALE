...
const createMockGenerateEmbeddingPort = (shouldThrow = false) => {
    return {
        generateEmbedding: async (question: AIQuestion): Promise<AIEmbedding> => {
            ...
            return new AIEmbedding(true, [0.1, 0.2, 0.3]); // Adjusted to match expected parameter types
        },
    };
};

Deno.test("Integration - GenerateEmbeddingUseCase - generateEmbedding - success", async () => {
...
Deno.test("Integration - GenerateEmbeddingUseCase - generateEmbedding - failure", async () => {
